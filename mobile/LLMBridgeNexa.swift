...
// Uncomment after adding NexaSdk.xcframework to Xcode project:
// import NexaSdk
...
final class LLMBridgeNexa: ObservableObject {
    ...
    func loadModel(_ model: NexaModel, backend: NexaBackend = .ane) async {
        ...
        // NexaSDK integration (uncomment after xcframework added):
        // do {
        ...
        // Stub for development without xcframework:
        try? await Task.sleep(nanoseconds: 800_000_000)
        isLoaded = true
        loadedModel = model
        statusMessage = "\(model.displayName) ready (\(backend.rawValue)) [STUB]"
    }
    ...
    func generate(prompt: String, maxTokens: Int = 512) async {
        ...
        let demoTokens = "GodLocal is running on-device via \(loadedModel?.displayName ?? "model"). No cloud. Full sovereignty. 🧠".components(separatedBy: " ")
        for token in demoTokens {
            output += token + " "
            tokensPerSecond = Float.random(in: 28...38)
